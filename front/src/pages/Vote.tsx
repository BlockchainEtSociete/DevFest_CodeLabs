...
import {
    fetchCompetitionsOfOneJury,
    fetchNomineesOfCompetition,
    voteOnCompetition
} from "../services/CompetitionService.service";
...
const Vote = () => {
    ...
    const onValidateVoteOnCompetition = async () => {
        if ( competitionToVote && nomineeToVote && await voteOnCompetition( competitionToVote, nomineeToVote, setLoading ) ) {
            ...
            try {
                const getCompetitions = await fetchCompetitionsOfOneJury( juryId, VotingCompetitionStatus.InProgress );
                getCompetitions && getCompetitions.length > 0 ? setCompetitions( getCompetitions ) : setCompetitions( [] );
            } catch ( err ) {
                setMessage( `Erreur lors de la récupération des compétitions en cours de vote : ${ err }` )
                setSeverity( "error" )
                setOpen( true )
                setLoading( false )
                setTimeout(
    ...
    return (
        <article>
            ...
            { ( competitions && competitions.length === 0 && !isLoading ) &&
                <h2
                >Aucunes compétitions ouvertes, soyez patients</h2> }
            <section style={ { display: 'flex', justifyContent: 'center', flexWrap: 'wrap' } }>
                { competitions && competitions.length > 0 && competitions.map( competition => (
                    <div style={ {
                        ...
                        border: "1px solid",
                        borderRadius: 20,
                        borderColor: `${ competitionToVote && competition.id === competitionToVote.id ? "blue" : "black" }`
                    } }
                         key={ competition.id } title="Cliquer pour voir les nominés"
                         onClick={ () => onCompetitionSelected( competition ) }>
                        <div>
                            <h4>{ competition.title }</h4>
                            <h5>Clotûre des votes
                                le { ( new Date( competition.endTime * 1000 ) ).toLocaleString( "fr" ) }</h5>
                            <img src={ competition.pictureUrl } alt={ competition.title } height='200' width='auto'/>
                        </div>
                    </div> ) ) }
            </section>
            { competitionToVote && nominees.length === 0 &&
                <h3>Aucuns nominés ({ competitionToVote.title }) pour le moment...</h3> }
                    ...
                    <section style={ { display: 'flex', justifyContent: 'center', flexWrap: 'wrap' } }>
                        { nominees.length > 0 && nominees.map( nominee => (
                            <div style={ { padding: 15, margin: 5, cursor: "pointer" } } key={ nominee.id }
                                 title="Cliquer pour voter pour ce nominé"
                                 onClick={ () => setNomineeToVote( nominee ) }>
                                <div>
                                    <h4>{ nominee.title }</h4>
                            ...
                            <div className="modal">
                                ...
                                <div className="actions">
                                    <button className="btn-validate"
                                            onClick={ () => onValidateVoteOnCompetition() }>Valider
                                    </button>
                                    <button className="btn-reset"
                                            onClick={ () => setNomineeToVote( undefined ) }>Annuler
                                    </button>
                                </div>
                            </div>
                        </Popup>
