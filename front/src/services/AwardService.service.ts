 ...
 */
export const fetchWinner = async (competitionId: number, typeCompetition: TypeCompetitions): Promise<{ winner: unknown, atLeastOneVote : boolean }> => {
    if (provider) { 
        try {
...
const getWinner = async (tokenIdNominee: number, typeCompetition: TypeCompetitions) => {
    ...
    if (typeCompetition === TypeCompetitions.Actor) {
        ...
        //movie
        winner = await fetchOneMovie(contractsInterface.contracts.Movies.address, contractsInterface.contracts.Movies.abi, tokenIdNominee);
    }
