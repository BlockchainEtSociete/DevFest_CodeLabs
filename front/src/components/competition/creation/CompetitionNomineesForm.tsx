...
import {
    addNomineesToCompetition,
    fetchEligibleNomineesByTypeCompetition,
    stopListenToNewPeopleAndMovieByTypeCompetition,
    listenToNewPeopleAndMovieByTypeCompetition
...
export const CompetitionNomineesForm = ( { typeCompetition, competitionId, setOpen, setMessage, setSeverity, isLoading, setIsLoading, onNomineesAdded }: CompetitionNomineesFormProps ) => {
    ...
    useEffect( () => {
        ( async () => {
            ...
            await listenToNewPeopleAndMovieByTypeCompetition( typeCompetition, ( nominee ) => setNominees( [ ...nominees, {
                nominee,
                isSelected: false
            } ] ) );
        } )();
    ...
    return (
        <div>
            <div style={ { display: 'flex', flexWrap: 'wrap', justifyContent: 'center' } }>
                {
                    nominees.map( ( { nominee: { tokenId, pictureUrl, title }, isSelected } ) => (
                        <div key={ `${ tokenId }` } onClick={ () => toggleNomineeSelection( tokenId ) }
            ...
            <button onClick={ onClickAddNomineesToCompetition } disabled={ isLoading }>Ajout des nominées de la
                compétition
            </button>
        </div>
    )
